...
echo "$($PSQL "SELECT DISTINCT name FROM games LEFT JOIN teams ON (games.winner_id = teams.team_id OR games.opponent_id = teams.team_id) WHERE year = 2014 AND round = 'Eighth-Final' ORDER BY name;")"
